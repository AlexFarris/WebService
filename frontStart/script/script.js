document.getElementById('addQuizForm').addEventListener('submit', function(event) {
    ...
    console.log('Invio del modulo in corso...'); // Aggiunto console.log per verificare che la funzione venga eseguita
    ...
    console.log('Raw:', raw); // Aggiunto console.log per verificare il corpo della richiesta
    ...
    console.log('Invio della richiesta POST...'); // Aggiunto console.log per verificare che la richiesta venga inviata

    fetch("https://baroniadelaide.alwaysdata.net/quiz", requestOptions)
        .then((response) => response.text())
        .then((result) => {
            console.log('Risposta ricevuta:', result); // Aggiunto console.log per verificare la risposta ricevuta
        })
        .catch((error) => console.error('Errore:', error)); // Aggiunto console.log per gestire eventuali errori
});
